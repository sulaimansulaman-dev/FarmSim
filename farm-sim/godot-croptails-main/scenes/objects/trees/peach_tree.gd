...
## A perennial peach tree. Near-identical to apple_tree.gd (same mechanic,
## different fruit/art) -- kept as a separate file rather than refactored
## together, to avoid risking already-tested trees this close to a deadline.
##
...
##
## peach_tree_fruiting.png is the same tree at the same four sizes, but with
## fruit/blossom art layered on top (12x5 grid, 48px cells). We only use its
## row 3 (full-grown) and row 4 (just-harvested) here.

const fruit_harvest_scene: PackedScene = preload("res://scenes/objects/trees/peach_harvest.tscn")
const growth_texture: Texture2D = preload("res://assets/game/objects/trees/fruit_tree_growth.png")
const fruiting_texture: Texture2D = preload("res://assets/game/objects/trees/peach_tree_fruiting.png")
...
## Row 4 of peach_tree_fruiting.png: fruit just fell, hearts still on the ground.
