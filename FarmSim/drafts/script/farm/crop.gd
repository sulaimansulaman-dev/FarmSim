...
var _library: DraftCropLibrary
var _definition: Dictionary = {}
var _rng := RandomNumberGenerator.new()


func _init(library: DraftCropLibrary, rng_seed: int = 0) -> void:
	_library = library
...
func plant(new_crop_id: String, starting_moisture: float = 0.5) -> bool:
	...
	if not _library.has_crop(new_crop_id):
		push_warning("DraftCrop.plant: unknown crop id '%s'" % new_crop_id)
		return false
