...
##
##   godot --headless --path . res://drafts/scenes/test/farm_world_test.tscn
##
...
const WORLD_SCENE := "res://drafts/scenes/farm/farm_world.tscn"
...
func _ready() -> void:
	...
	_check("E on bare soil plants straight away", world._tiles[0].state == DraftCrop.State.GROWING)
	...
	var bare := DraftCrop.new(world._library, 99)
	...
	world._context_action()
	_check("E does NOT harvest a ripe crop", world._tiles[0].state == DraftCrop.State.MATURE)
	...
	_check("H harvests it", world._tiles[0].state == DraftCrop.State.HARVESTED)
	world._refresh()
	...
	world._context_action()
	_check("a crop is growing again", world._tiles[2].state == DraftCrop.State.GROWING)
	...
	for i in range(world._tiles.size()):
		world._tiles[i] = DraftCrop.new(world._library, i)
	...
	_check("planting broke leaves the plot empty", world._tiles[3].state == DraftCrop.State.EMPTY)
