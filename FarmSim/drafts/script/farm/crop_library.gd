...
const DATA_PATH := "res://drafts/data/crops.json"
...
func load_from(path: String = DATA_PATH) -> bool:
	...
	if not FileAccess.file_exists(path):
		load_error = "DraftCrop data file not found at %s" % path
		return false
	...
	if parsed == null or typeof(parsed) != TYPE_DICTIONARY:
		load_error = "DraftCrop data at %s is not valid JSON." % path
		return false

	if not parsed.has("crops") or typeof(parsed["crops"]) != TYPE_ARRAY:
		load_error = "DraftCrop data is missing a 'crops' array."
		return false
	...
	if _crops.is_empty():
		load_error = "DraftCrop data contains no crops."
		return false
...
func sprite_sheet_path() -> String:
	return str(sprites.get("sheet", "res://drafts/game/objects/basic_plants.png"))
...
func stage_sprite_region(crop: DraftCrop) -> Rect2:
...
func _validate_crop(entry) -> String:
	...
	for field in ["id", "display_name", "base_yield_kg", "daily_water_use", "wilting_point", "stages"]:
		if not entry.has(field):
			return "DraftCrop '%s' is missing required field '%s'." % [entry.get("id", "<no id>"), field]

	if typeof(entry["stages"]) != TYPE_ARRAY or entry["stages"].is_empty():
		return "DraftCrop '%s' must define at least one growth stage." % entry["id"]

	for stage in entry["stages"]:
		for field in ["id", "display_name", "days", "water_sensitivity"]:
			if not stage.has(field):
				return "DraftCrop '%s' has a stage missing required field '%s'." % [entry["id"], field]
		if int(stage["days"]) <= 0:
			return "DraftCrop '%s' stage '%s' must last at least one day." % [entry["id"], stage["id"]]
