...
##
##   godot --headless --script res://drafts/script/farm/crop_sim_demo.gd
##
...
func _initialize() -> void:
	var library := DraftCropLibrary.new()
...
func _compare_seasons(library: DraftCropLibrary) -> void:
	_header("SCENARIO 4 - same maize, same care, four seasons")
	print("")

	var weather := DraftWeatherSystem.new(7)
	...
	for season_id in weather.season_ids():
		...
		for run in range(runs):
			var season_weather := DraftWeatherSystem.new(run * 31 + 7)
			season_weather.load_from()
			season_weather.set_season(season_id)

			var crop := DraftCrop.new(library, run * 31 + 7)
			crop.plant("maize", 0.55)

			var days := 0
			while not crop.is_ready_to_harvest() and crop.state != DraftCrop.State.DEAD and days < 200:
...
func weather_suits(season_id: String) -> bool:
	var probe := DraftWeatherSystem.new(0)
	probe.load_from()
...
func _run_attentive_farmer(library: DraftCropLibrary) -> void:
	_header("SCENARIO 1 - attentive farmer, maize")
	var crop := DraftCrop.new(library)
	crop.plant("maize", 0.6)

	while not crop.is_ready_to_harvest() and crop.state != DraftCrop.State.DEAD:
		if crop.moisture < 0.45:
			crop.water(0.35)
		crop.advance_day()
...
func _run_neglectful_farmer(library: DraftCropLibrary) -> void:
...
func _run_untreated_pest(library: DraftCropLibrary) -> void:
	...
	var crop := DraftCrop.new(library, 12345)
...
func _print_summary(summary: Dictionary) -> void:
	print("")
	print("  DraftCrop:            %s" % summary["display_name"])
