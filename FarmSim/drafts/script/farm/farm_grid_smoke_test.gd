...
##
##   godot --headless --path . res://drafts/scenes/test/farm_grid_test.tscn
##
...
const GRID_SCENE := "res://drafts/scenes/farm/farm_grid.tscn"
...
func _ready() -> void:
	...
	_check("tile 1 is growing", crop.state == DraftCrop.State.GROWING)
	...
	grid._on_tile_pressed(0)
	_check("cannot harvest an immature crop", crop.state != DraftCrop.State.HARVESTED)
	...
	_check("tile is freed after harvest", crop.state == DraftCrop.State.HARVESTED)
	_check(
	...
	for i in range(40):
		if neglected.is_ready_to_harvest() or neglected.state == DraftCrop.State.DEAD:
			break
		grid._on_end_day()
	_check(
		"a neglected crop is punished",
		neglected.state == DraftCrop.State.DEAD or neglected.total_penalty_percent() > 10.0
	)
