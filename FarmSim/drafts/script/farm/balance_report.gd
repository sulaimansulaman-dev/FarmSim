...
##
##   godot --headless --path . res://drafts/scenes/test/balance_report.tscn
##
...
func run(lib, threshold: float, treat: bool, water_per_day: int, seed_v: int, heed_forecast: bool = false) -> Dictionary:
	var w := DraftWeatherSystem.new(seed_v); w.load_from(); w.set_season("summer")
	var crops: Array = []
	for i in range(12):
		var c := DraftCrop.new(lib, seed_v * 100 + i)
	...
	for day in range(200):
		var alive := 0
		for c in crops:
			if c.state == DraftCrop.State.GROWING: alive += 1
		if alive == 0: break

		# The player only manages so many plots a day.
		var watered := 0
		for c in crops:
			if c.state != DraftCrop.State.GROWING: continue
		...
		for c in crops:
			if c.state != DraftCrop.State.GROWING and c.state != DraftCrop.State.MATURE: continue
	...
	for c in crops:
		if c.state == DraftCrop.State.DEAD: dead += 1
		if c.state == DraftCrop.State.MATURE: ripe += 1
...
func _ready() -> void:
	var lib := DraftCropLibrary.new(); lib.load_from()
