...
const PLAYER_SCENE := "res://drafts/scenes/characters/player/player.tscn"
const SOIL_SHEET := "res://drafts/game/tilesets/tilled_dirt.png"
const GRASS_SHEET := "res://drafts/game/tilesets/grass.png"
...
## planted at its root.
const PEST_SPRITE := "res://drafts/game/objects/pest_caterpillar.png"
const TOOL_SHEET := "res://drafts/game/objects/basic_tools_and_meterials.png"
...
var _library := DraftCropLibrary.new()
var _weather := DraftWeatherSystem.new(2026)
var _prices := DraftPriceList.new()
...
func _build_plots() -> void:
	...
	for i in range(PLOT_COLUMNS * PLOT_ROWS):
		...
		_tiles.append(DraftCrop.new(_library, 1000 + i))
...
func _has_crops_in_the_ground() -> bool:
	for crop in _tiles:
		if crop.state == DraftCrop.State.GROWING or crop.state == DraftCrop.State.MATURE:
			return true
	return false
...
func _new_season() -> void:
	...
	for i in range(_tiles.size()):
		_tiles[i] = DraftCrop.new(_library, 1000 + i)
	_weather.advance()
...
func _update_plot_label() -> void:
	...
	var crop: DraftCrop = _tiles[_nearest_plot]
	var name := "Plot %d" % (_nearest_plot + 1)

	match crop.state:
		DraftCrop.State.EMPTY, DraftCrop.State.HARVESTED:
...
func _context_action() -> void:
	...
	match crop.state:
		...
		DraftCrop.State.DEAD:
			_tiles[_nearest_plot] = DraftCrop.new(_library, 1000 + _nearest_plot)
...
func _water_nearest() -> void:
	...
	elif crop.state == DraftCrop.State.GROWING or crop.state == DraftCrop.State.MATURE:
...
func _harvest_nearest() -> void:
	...
	if not crop.is_ready_to_harvest():
		if crop.state == DraftCrop.State.GROWING:
	...
	# Placeholder economics - see DraftPriceList. The Economy developer owns DEL-05.
...
func _plant_at(index: int) -> void:
	var crop: DraftCrop = _tiles[index]
	if crop.state == DraftCrop.State.HARVESTED:
		_tiles[index] = DraftCrop.new(_library, 1000 + index)
		crop = _tiles[index]
...
func _end_day() -> void:
	...
	for i in range(_tiles.size()):
		var crop: DraftCrop = _tiles[i]
		if crop.state != DraftCrop.State.GROWING and crop.state != DraftCrop.State.MATURE:
			continue
		...
		var was_growing := crop.state == DraftCrop.State.GROWING
		...
		if crop.state == DraftCrop.State.DEAD:
			events.append("[color=#e77]Plot %d died.[/color]" % (i + 1))
		elif crop.state == DraftCrop.State.MATURE and was_growing:
...
func _refresh_plot(index: int) -> void:
	...
	if crop.state == DraftCrop.State.EMPTY or crop.state == DraftCrop.State.HARVESTED:
		plant.visible = false
		return
	...
	if crop.state == DraftCrop.State.DEAD:
		...
		var health_ratio := crop.health / DraftCrop.MAX_HEALTH
